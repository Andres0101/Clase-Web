...
<body>
			...
			<nav id="navDos">
					...
					<li>
						<div class="formularioDos">
							<form action="carroCompras.php" method="POST">
	...
	<div id="productos">
		...
		<?php
			...
			//Obtengo el id de los productos que están en el carrito de compras
			$productoAComprar = "SELECT carrito.idProducto FROM carrito WHERE carrito.idUsuario='".$idUsuario."'";

			$resultAComprar = mysqli_query($conexion, $productoAComprar);

			while($otheRow = mysqli_fetch_array($resultAComprar)) {
				//Obtiene el id del producto
				$idProducto = $otheRow['idProducto'];
			}

			//Selecciona los productos que están en el carrito del usuario
			$productosCarrito = "SELECT productos.nombreProducto, productos.descripcionProducto, productos.imagenProducto FROM productos
				JOIN carrito ON carrito.idProducto=productos.idProducto
				JOIN usuarios ON usuarios.idUsuario=carrito.idUsuario
			...
			if($_GET != null) {
				$idProductoSeleccionado = $_GET['idProducto'];
					...
					//Agrega a la base de datos de compras el producto del carrito que el usuario compró
					$agregarProductosComprados = "INSERT INTO `compras`(`idUsuario`, `idProducto`, `fecha`)
					VALUES ('".$idUsuario."', '".$idProductoSeleccionado."', CURDATE())";
							
					$resultadoProductosComprados = mysqli_query($conexion, $agregarProductosComprados);
			}
		   	...
		   	while($row = mysqli_fetch_array($resultCarrito)) {
			   	...
			   	echo "<article>";
					echo "<div class='contenido'";
					    ...
					    //Selecciono el nombre de los productos que están en el carrito
						...
						echo "<div class='contenido2'>";							
							echo "<a href='carroCompras.php?idProducto=".$idProducto."'>"; //Escribe en la url el id del producto				
